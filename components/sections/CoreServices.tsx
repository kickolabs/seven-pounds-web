...
export default function CoreServices() {
  ...
  const prev = () => go((active - 1 + CORE_SERVICES.length) % CORE_SERVICES.length)
  const next = useCallback(() => go((active + 1) % CORE_SERVICES.length), [active, go])
          ...
          <div className="relative h-[420px] flex items-center">
            {CORE_SERVICES.map((service, i) => (
              <div
                ...
                className="absolute inset-0 flex flex-col justify-center text-left opacity-0 translate-y-12"
              >
